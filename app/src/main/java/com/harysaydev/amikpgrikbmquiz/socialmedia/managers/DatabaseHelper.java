...
public class DatabaseHelper {
    ...
    StorageReference riversRef,mRefku;
...
//        database.setPersistenceEnabled(true);
    ...
    public UploadTask uploadImage(Uri uri, String imageTitle) {
        ...
        //uploadTask = FirebaseStorage.getInstance().getReference().child(IMAGES_STORAGE_KEY + "/" + imageTitle).putFile();
        // Create file metadata including the content type
        StorageMetadata metadata = new StorageMetadata.Builder()
                .setCacheControl("max-age=7776000, Expires=7776000, public, must-revalidate")
                .build();
        uploadTask = FirebaseStorage.getInstance().getReference().child(IMAGES_STORAGE_KEY + "/" + imageTitle).putFile(uri,metadata);
        return uploadTask;
    }
    public UploadTask uploadImage2(Uri uri, String imageTitle) {
        riversRef = getStorageReference().child(IMAGES_STORAGE_KEY + "/" + imageTitle);
        // Create file metadata including the content type
        StorageMetadata metadata = new StorageMetadata.Builder()
                .setCacheControl("max-age=7776000, Expires=7776000, public, must-revalidate")
                .build();
