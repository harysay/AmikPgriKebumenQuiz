...
import com.harysaydev.amikpgrikbmquiz.socialmedia.managers.listeners.OnCountChangedListener;
...
import com.harysaydev.amikpgrikbmquiz.socialmedia.managers.listeners.OnRequestComplete;
import com.harysaydev.amikpgrikbmquiz.socialmedia.model.Follower;
import com.harysaydev.amikpgrikbmquiz.socialmedia.model.Following;
...
public class FollowInteractor {
    ...
    public void getFollowingPosts(String userId, OnDataChangedListener<FollowingPost> listener) {
        //databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOWINGS_POSTS_DB_KEY).child(userId)
        FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOWINGS_POSTS_DB_KEY).child(userId)
    ...
    private DatabaseReference getFollowersRef(String userId) {
        //return databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(userId).child(DatabaseHelper.FOLLOWERS_DB_KEY);
        return FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(userId).child(DatabaseHelper.FOLLOWERS_DB_KEY);
    }

    private DatabaseReference getFollowingsRef(String userId) {
        //return databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(userId).child(DatabaseHelper.FOLLOWINGS_DB_KEY);
        return FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(userId).child(DatabaseHelper.FOLLOWINGS_DB_KEY);
    }
    ...
    public void isFollowingExist(String followerUserId, String followingUserId, final OnObjectExistListener onObjectExistListener) {
        //DatabaseReference followingRef = databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followerUserId).child(DatabaseHelper.FOLLOWINGS_DB_KEY).child(followingUserId);
        DatabaseReference followingRef = FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followerUserId).child(DatabaseHelper.FOLLOWINGS_DB_KEY).child(followingUserId);
        //DatabaseReference followerRef = databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followingUserId).child(DatabaseHelper.FOLLOWERS_DB_KEY).child(followerUserId);
        DatabaseReference followerRef = FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followingUserId).child(DatabaseHelper.FOLLOWERS_DB_KEY).child(followerUserId);
    ...
    private Task<Void> addFollowing(String followerUserId, String followingUserId) {
        ...
        //return databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followerUserId).child(DatabaseHelper.FOLLOWINGS_DB_KEY).child(followingUserId).setValue(following);
        return FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followerUserId).child(DatabaseHelper.FOLLOWINGS_DB_KEY).child(followingUserId).setValue(following);
    }

    private Task<Void> addFollower(String followerUserId, String followingUserId) {
        Follower follower = new Follower(followerUserId);
        //return databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followingUserId).child(DatabaseHelper.FOLLOWERS_DB_KEY).child(followerUserId).setValue(follower);
        return FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followingUserId).child(DatabaseHelper.FOLLOWERS_DB_KEY).child(followerUserId).setValue(follower);
    }

    private Task<Void> removeFollowing(String followerUserId, String followingUserId) {
        //return databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followerUserId).child(DatabaseHelper.FOLLOWINGS_DB_KEY).child(followingUserId).removeValue();
        return FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followerUserId).child(DatabaseHelper.FOLLOWINGS_DB_KEY).child(followingUserId).removeValue();
    }

    private Task<Void> removeFollower(String followerUserId, String followingUserId) {
        //return databaseHelper.getDatabaseReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followingUserId).child(DatabaseHelper.FOLLOWERS_DB_KEY).child(followerUserId).removeValue();
        return FirebaseDatabase.getInstance().getReference().child(DatabaseHelper.FOLLOW_DB_KEY).child(followingUserId).child(DatabaseHelper.FOLLOWERS_DB_KEY).child(followerUserId).removeValue();
    }
