 ...
 */
public class PemBerorientasiObjekUas extends SQLiteOpenHelper {
    private static final String Database_path = "/data/data/com.harysaydev.amikpgrikbmquiz/databases/";
    private static final String Database_name = "pbouas.db";//NAME of database stored in Assets folder
    private static final String Table_name = "pbo";//name of table
    ...
    public PemBerorientasiObjekUas(Context context) {//constructor
...
//        if(CheckIsDataAlreadyInDBorNot(Table_name,uidSqliteFromweb,idFromWeb)==""){
//            String SQLiteDataBaseQueryHolder = "INSERT INTO "+Table_name+" (Idfromweb,Question,OptionA,OptionB,OptionC,OptionD,Answer,Conclusion) VALUES('"+idFromWeb+"','"+pertanyaan+"','"+optionA+"','"+optionB+"','"+optionC+"','"+optionD+"','"+answer+"','"+conclusion+"');";
//            sqlite.execSQL(SQLiteDataBaseQueryHolder);
//        }
    }
    ...
    public String cekDuplikasi(String id_dariWeb)//Used to read the data from the Des.db file where id is given and we choose id randomly
    {
        ...
        Cursor cruDupPbo = sqlite.rawQuery("SELECT " + uid + " FROM " + Table_name + " WHERE " + uid + " = " + id_dariWeb + "", null);//cursor to that query
        if (cruDupPbo.getCount()>0) {
            duplikasi = "true";
        }
        else {
...
//    public String CheckIsDataAlreadyInDBorNot(String TableName, String idfieldSqlite, String fieldValueWeb) {
//        //SQLiteDatabase sqldb = EGLifeStyleApplication.sqLiteDatabase;
//        String kodeSama;
//        //String Query = "Select * from " + TableName + " where " + idfieldSqlite + " = " + fieldValueWeb;
//        Cursor mCount= sqlite.rawQuery("SELECT " + uid + " FROM " + TableName +" WHERE " + uid + " = '" + fieldValueWeb +"'", null);
//        if (mCount.moveToFirst())
//            kodeSama = mCount.getString(0);
//        else
//            kodeSama = "false";
//        return kodeSama;
////        mCount.moveToFirst();
